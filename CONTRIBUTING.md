...
This example is based on Ubuntu 24.04, but it should work on many versions of Debian and Ubuntu.
...
```
pgbackrest/test/test.py --module=postgres

--- output ---

    P00   INFO: test command begin 2.60.0dev: --module=postgres --no-log-timestamp
    P00   INFO: test begin on aarch64 - log level info
    P00   INFO: cleanup old data
    P00   INFO: autogenerate code
    P00   INFO: build for none (/home/vagrant/test/build/none)
...
Build a container to run tests. The vm must be pre-configured but a variety are available. A vagrant file is provided in the test directory as an example of running in a virtual environment. The vm names are all three character abbreviations, e.g. `u24` for Ubuntu 24.04.
...
```
pgbackrest/test/test.py --build-only
```
