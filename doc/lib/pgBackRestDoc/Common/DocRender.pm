...
sub new
{
    ...
    if (defined($self->{strRenderOutKey}))
    {
        ...
        # Register the section anchor map so cross-page links to this page can be verified
        $self->{oManifest}->{hPageAnchor}{$self->{strRenderOutKey}} = $self->sectionAnchorMap();
    }
...
# sectionAnchorMap
#
# Build the set of section anchors for this page. An anchor is the section id prefixed with the parent anchors, which matches the
# anchor generated by the html renderer. A section with anchor set to no-inherit does not include its parent anchors.
####################################################################################################################################
sub sectionAnchorMap
{
    my $self = shift;

    my $rhAnchor = {};

    foreach my $strPath (keys(%{$self->{oSection}}))
    {
        my @stryId;
        my $oSection = ${$self->{oSection}}{$strPath};

        # Prepend section ids walking up the parents until a section that does not inherit the parent anchor
        while (defined($oSection))
        {
            unshift(@stryId, $oSection->paramGet('id'));

            last if $oSection->paramTest(XML_SECTION_PARAM_ANCHOR, XML_SECTION_PARAM_ANCHOR_VALUE_NOINHERIT);

            my $strParentPath = $oSection->paramGet('path-parent', false);
            $oSection = defined($strParentPath) ? ${$self->{oSection}}{$strParentPath} : undef;
        }

        $rhAnchor->{join('/', @stryId)} = true;
    }
...
sub processTag
{
    ...
    if ($strTag eq 'link')
    {
        ...
        if (!defined($strUrl))
        {
            ...
            # A section link must begin with / since the leading / is stripped to form the anchor
            if (defined($strSection) && substr($strSection, 0, 1) ne '/')
            {
                confess &log(ERROR, "link section '${strSection}' must begin with '/'");
            }
            ...
            if (defined($strPage))
            {
                # Record the page/section link so it can be verified once all pages have been rendered and their section
                # anchors are known
                if (defined($strSection))
                {
                    push(
                        @{$self->{oManifest}->{rhyLinkVerify}},
                        {source => $self->{strRenderOutKey}, page => $strPage, section => $strSection});
                }
