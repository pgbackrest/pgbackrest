...
verifyFile(
    ...
    const Buffer *const fileChecksum, const uint64_t fileSize, const CipherInfo *const cipherInfo)
{
    FUNCTION_LOG_BEGIN(logLevelDebug);
        ...
        FUNCTION_LOG_PARAM(CIPHER_INFO, cipherInfo);                // Info to access the repo file if encrypted
    FUNCTION_LOG_END();
    ...
    MEM_CONTEXT_TEMP_BEGIN()
    {
        ...
        if (cipherInfoType(cipherInfo) != cipherTypeNone)
            ioFilterGroupAdd(filterGroup, cipherBlockNewP(cipherModeDecrypt, cipherInfo));
