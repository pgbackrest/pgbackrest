...
stanzaStatusLockAdd(KeyValue *targetKv, const Variant *const lockKey, const InfoStanzaLock *lock)
{
    ...
    if (lock->size != 0)
    {
        ...
        if (cfgOptionStrId(cfgOptOutput) != CFGOPTVAL_OUTPUT_JSON)
            kvPut(lockKv, STATUS_KEY_LOCK_PERCENT_COMPLETE_VAR,
                  VARUINT((unsigned int)(((double)lock->sizeComplete / (double)lock->size) * 10000)));
    }
}
...
infoUpdateStanzaLock(
    InfoStanzaLock *const stanzaLock, const String *const stanzaName, const unsigned int repoIdx,
