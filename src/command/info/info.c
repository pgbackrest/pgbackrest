...
stanzaInfoList(const String *stanza, StringList *stanzaList, const String *backupLabel)
{
    ...
    for (unsigned int idx = 0; idx < strLstSize(stanzaList); idx++)
    {
        ...
        static bool runningBackup = false;
        ...
        {
            stanzaStatus(INFO_STANZA_STATUS_CODE_NO_BACKUP, INFO_STANZA_STATUS_MESSAGE_NO_BACKUP_STR, runningBackup, stanzaInfo);
        }

        // If a status has still not been set then set it to OK
        if (kvGet(varKv(stanzaInfo), STANZA_KEY_STATUS_VAR) == NULL)
            stanzaStatus(INFO_STANZA_STATUS_CODE_OK, INFO_STANZA_STATUS_MESSAGE_OK_STR, runningBackup, stanzaInfo);
    ...
    if (stanza != NULL && !stanzaFound)
    {
        ...
        stanzaStatus(INFO_STANZA_STATUS_CODE_MISSING_STANZA_PATH, INFO_STANZA_STATUS_MESSAGE_MISSING_STANZA_PATH_STR, false, stanzaInfo);
        varLstAdd(result, stanzaInfo);
    }
                    ...
                    if (statusCode != INFO_STANZA_STATUS_CODE_OK)
                    {
                       if (varBool(kvGet(backupLockKv, STATUS_KEY_LOCK_BACKUP_HELD_VAR)))
                       {
                           strCatFmt(
                               ...
                               strPtr(varStr(STATUS_KEY_LOCK_BACKUP_STR)));
                       }
                       else
                       {
                           strCatFmt(
