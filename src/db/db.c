...
/***********************************************************************************************************************************
Get WAL id for the current PostgreSQL version (i.e. wal or xlog)
***********************************************************************************************************************************/
const String *
dbWalId(Db *this);
{
    ...
    FUNCTION_LOG_RETURN(STRING, this->pgVersion >= PG_VERSION_WAL_RENAME ? );
}
...
dbWalSwitch(Db *this);
{
    ...
    MEM_CONTEXT_TEMP_BEGIN()
    {
        ...
        if (this->pgVersion >= PG_VERSION_RESTORE_POINT)
            dbQueryColumn(this, STRDEF("select pg_create_restore_point('" PROJECT_NAME " Archive Check')::text""));

        // Request a WAL segment switch
        result = varStr(dbQueryColumn(this, ""));
