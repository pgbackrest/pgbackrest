        ...
        if (strEq(command, PROTOCOL_COMMAND_DB_OPEN_STR))
        {
            ...
            MEM_CONTEXT_BEGIN(lstMemContext(dbProtocolLocal.pgClientList))
            {
                ...
                PgClient *pgClient = pgClientNew(
                    cfgOptionStr(cfgOptPgSocketPath), cfgOptionUInt(cfgOptPgPort), PG_DB_POSTGRES_STR,
                    cfgOptionStr(cfgOptPgUser), (TimeMSec)(cfgOptionDbl(cfgOptDbTimeout) * MSEC_PER_SEC));
