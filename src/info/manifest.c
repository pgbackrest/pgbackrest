...
manifestSaveCallback(void *callbackData, const String *sectionNext, InfoSave *infoSaveData)
{
    ...
    {
        // These values are saved only for compatibility with older versions and are not loaded in this version
        infoSaveValue(
            infoSaveData, MANIFEST_SECTION_BACKUP_DB_STR, STRDEF("db-catalog-version"),
            jsonFromUInt(pgVersionInfo(manifest->pub.data.pgVersion).catalogVersion));
        infoSaveValue(
            infoSaveData, MANIFEST_SECTION_BACKUP_DB_STR, STRDEF("db-control-version"),
            jsonFromUInt(pgVersionInfo(manifest->pub.data.pgVersion).controlVersion));
