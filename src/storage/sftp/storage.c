...
storageSftpLibSsh2SessionFreeResource(THIS_VOID)
{
    ...
    if (this->sftpHandle != NULL)
    {
        ...
        while (rc == LIBSSH2_ERROR_EAGAIN && storageSftpWaitFd(this));
    ...
    else
    {
        ...
        if (result.level >= storageInfoLevelDetail)
        {
            ...
            if (result.type == storageTypeLink)
            {
                ...
                while (linkDestinationSize == LIBSSH2_ERROR_EAGAIN && storageSftpWaitFd(this));
...
storageSftpList(THIS_VOID, const String *const path, const StorageInfoLevel level, const StorageInterfaceListParam param)
{
    ...
    while (sftpHandle == NULL && libssh2_session_last_errno(this->session) == LIBSSH2_ERROR_EAGAIN && storageSftpWaitFd(this));
    ...
    else
    {
        ...
        TRY_BEGIN()
        {
            MEM_CONTEXT_TEMP_RESET_BEGIN()
            {
                ...
                while (len > 0 || (len == LIBSSH2_ERROR_EAGAIN && storageSftpWaitFd(this)));
            }
...
storageSftpNewRead(THIS_VOID, const String *const file, const bool ignoreMissing, const StorageInterfaceNewReadParam param)
{
    ...
    FUNCTION_LOG_RETURN(
        STORAGE_READ,
        storageReadSftpNew(
            this, file, ignoreMissing, this->session, this->sftpSession, this->sftpHandle,
...
storageSftpNewWrite(THIS_VOID, const String *const file, const StorageInterfaceNewWriteParam param)
{
    ...
    FUNCTION_LOG_RETURN(
        STORAGE_WRITE,
        storageWriteSftpNew(
            this, file, this->session, this->sftpSession, this->sftpHandle, param.modeFile,
    ...
    const TimeMSec timeout, const String *const keyPriv, const StringId hostKeyHashType, const StorageSftpNewParam param)
{
    ...
    OBJ_NEW_BEGIN(StorageSftp, .childQty = MEM_CONTEXT_QTY_MAX, .callbackQty = 1)
    {
        *this = (StorageSftp)
        ...
        while (handshakeStatus == LIBSSH2_ERROR_EAGAIN && storageSftpWaitFd(this));

        if (handshakeStatus == LIBSSH2_ERROR_EAGAIN)
            THROW_FMT(ServiceError, "timeout during libssh2 handshake [%d]", handshakeStatus);
