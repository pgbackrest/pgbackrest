...
FV_EXTERN const String *cfgOptionDisplayVar(const Variant *const value, const ConfigOptionType optionType);
...
FV_EXTERN unsigned int cfgOptionKeyToIdx(ConfigOption optionId, unsigned int key);

// Total indexes for the option if in a group, 1 otherwise.
FV_EXTERN unsigned int cfgOptionIdxTotal(ConfigOption optionId);

// Get config option as a Variant
FV_EXTERN Variant *cfgOptionIdxVar(ConfigOption optionId, unsigned int optionIdx);
...
FV_EXTERN void cfgOptionInvalidate(ConfigOption optionId);
