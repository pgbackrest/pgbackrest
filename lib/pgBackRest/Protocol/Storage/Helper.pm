...
use constant STORAGE_REPO_BACKUP_TMP                                => '<REPO:BACKUP:TMP>';
    push @EXPORT, qw(STORAGE_REPO_BACKUP_TMP);
...
sub storageRepoRule
{
    ...
    # Return backup path
    elsif ($strRule eq STORAGE_REPO_BACKUP_TMP)
    {
        $strResultFile = "temp/${strStanza}.tmp" . (defined($strFile) ? "/${strFile}" : '');
    }
    # Else error
    else
    {
