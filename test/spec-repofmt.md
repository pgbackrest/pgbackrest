...
- The two info files are saved separately, so an upgrade interrupted between them leaves them at different formats. The higher of the two is then the only target that does not downgrade a file, so the upgrade writes it to both whether or not a format was requested, and re-running the command repairs the stanza. That is what the version and system id they also carry have always done. The repair is warned about since the run that left the formats apart did not get far enough to report anything.
