...
testRun(void)
{
    ...
    if (testBegin("Set expected WAL segment size"))
    {
        // -------------------------------------------------------------------------------------------------------------------------
        TEST_TITLE("Unexpected WAL segment size");

        StringList *argList = strLstNew();
        hrnCfgArgRawZ(argList, cfgOptPgWalSegmentSize, "64MiB");
        hrnCfgArgRawZ(argList, cfgOptStanza, "test");
        hrnCfgArgKeyRawZ(argList, cfgOptPgPath, 1, "/pg1");
        hrnCfgArgKeyRawZ(argList, cfgOptRepoRetentionFull, 1, "2");
        hrnCfgLoadP(cfgCmdBackup, argList);

        HRN_PG_CONTROL_PUT(storageTest, PG_VERSION_12);

        TEST_ERROR(
            pgControlFromFile(storageTest, NULL), FormatError,
            "Expected WAL segment size is 67108864 but actual is 16777216");

        // -------------------------------------------------------------------------------------------------------------------------
        TEST_TITLE("Invalid WAL segment size for Postgres 9.4");

        HRN_PG_CONTROL_PUT(storageTest, PG_VERSION_94, .walSegmentSize = 128 * 1024 * 1024);

        TEST_ERROR(
            pgControlFromFile(storageTest, NULL), FormatError,
            "WAL segment size is 134217728 but must be less then or equal to 67108864 for PostgreSQL < 10");

        // -------------------------------------------------------------------------------------------------------------------------
        TEST_TITLE("Expected WAL segment size for Postgres 10+");

        HRN_PG_CONTROL_PUT(
            storageTest, PG_VERSION_12, .systemId = 0xFACEFACE, .checkpoint = 0xEEFFEEFFAABBAABB, .timeline = 47,
        ...
        // -------------------------------------------------------------------------------------------------------------------------
        TEST_TITLE("Expected WAL segment size for Postgres 9.4");

        HRN_PG_CONTROL_PUT(
            storageTest, PG_VERSION_94, .systemId = 0xFACEFACE, .checkpoint = 0xEEFFEEFFAABBAABB, .timeline = 47,
            .walSegmentSize = 64 * 1024 * 1024);

        PgControl info2 = {0};
        TEST_ASSIGN(info2, pgControlFromFile(storageTest, NULL), "get control info v11");
        TEST_RESULT_UINT(info2.systemId, 0xFACEFACE, "   check system id");
        TEST_RESULT_UINT(info2.version, PG_VERSION_94, "   check version");
        TEST_RESULT_UINT(info2.catalogVersion, 201409291, "   check catalog version");
        TEST_RESULT_UINT(info2.checkpoint, 0xEEFFEEFFAABBAABB, "check checkpoint");
        TEST_RESULT_UINT(info2.timeline, 47, "check timeline");
    }
