...
testRun(void)
{
    ...
    if (testBegin("storageNew()"))
    {
        ...
        TEST_ERROR(
            storageSftpNewP(TEST_PATH_STR, STRDEF("localhost"), 22, 5, 5, .user = TEST_USER_STR, .keyPriv = KEYPRIV,
                .keyPub = KEYPUB, .hostkeyHash = hashTypeSha1),
        ...
        TEST_ERROR(storageSftpNewP(
            TEST_PATH_STR, STRDEF("localhost"), 22, 20, 20, .user = TEST_USER_STR, .keyPriv = KEYPRIV, .keyPub = KEYPUB,
                .write = true, .hostkeyHash = cipherTypeAes256Cbc),
        ...
        TEST_ERROR(
            storageSftpNewP(
                TEST_PATH_STR, STRDEF("localhost"), 22, 1000, 1000, .user = TEST_USER_STR, .keyPriv = KEYPRIV, .keyPub = KEYPUB,
                .keyPassphrase = STRDEF("keyPassphrase"), .hostkeyHash = hashTypeSha1),
            ServiceError,
            "public key authentication failed: libssh2 error [-16]\n"
            "HINT: libssh2 compiled against non-openssl libraries requires --repo-sftp-private-keyfile and"
                " --repo-sftp-public-keyfile to be provided\n"
        ...
        TEST_ASSIGN(
            ...
            storageSftpNewP(
                ...
                .keyPub = KEYPUB, .hostkeyHash = hashTypeSha1, .modeFile = 0600, .modePath = 0700, .write = true),
    ...
    if (testBegin("storageExists() and storagePathExists()"))
    {
        ...
        Storage *storageTest = storageSftpNewP(
            ...
            .write = true, .hostkeyHash = hashTypeSha1);
    ...
    if (testBegin("storagePath()"))
    {
        ...
        TEST_ASSIGN(
            ...
            storageSftpNewP(
                STRDEF("/path/to"), STRDEF("localhost"), 22, 1000, 1000, .user = TEST_USER_STR, .keyPriv = KEYPRIV,
                .keyPub = KEYPUB, .hostkeyHash = hashTypeSha1, .pathExpressionFunction = storageTestPathExpression),
            "new storage /path/to");
    ...
    if (testBegin("storageLinkCreate()"))
    {
        ...
        TEST_ASSIGN(
            ...
            storageSftpNewP(
                ...
                .keyPriv = KEYPRIV, .keyPub = KEYPUB, .hostkeyHash = hashTypeSha1),
            "new storage /");
    ...
    if (testBegin("storageNewWrite()"))
    {
        ...
        TEST_ASSIGN(
            file,
            storageNewWriteP(storageTest, fileNoPerm, .noAtomic = true, .noCreatePath = true), "new write file");
        ...
        TEST_ASSIGN(
            file,
            storageNewWriteP(storageTest, STRDEF("missing"), .noAtomic = true, .noCreatePath = true), "new write file");
        ...
        hrnLibSsh2ScriptSet((HrnLibSsh2 [])
        {
            ...
            {.function = HRNLIBSSH2_SFTP_FSTAT_EX, .param = "[1]", .resultInt = LIBSSH2_ERROR_NONE,
                ...
                .flags = LIBSSH2_SFTP_ATTR_PERMISSIONS | LIBSSH2_SFTP_ATTR_ACMODTIME | LIBSSH2_SFTP_ATTR_UIDGID |
                    LIBSSH2_SFTP_ATTR_SIZE,
                .mtime = 1, .uid = TEST_USER_ID, .gid = TEST_GROUP_ID, .filesize = 8},
            {.function = HRNLIBSSH2_SFTP_FSYNC, .resultInt = LIBSSH2_ERROR_EAGAIN},
            {.function = HRNLIBSSH2_SFTP_FSYNC, .resultInt = LIBSSH2_ERROR_NONE},
            {.function = HRNLIBSSH2_SFTP_FSTAT_EX, .param = "[1]", .resultInt = LIBSSH2_ERROR_NONE,
                .attrPerms = LIBSSH2_SFTP_S_IFREG | LIBSSH2_SFTP_S_IRUSR | LIBSSH2_SFTP_S_IWUSR | LIBSSH2_SFTP_S_IRGRP,
        ...
        hrnLibSsh2ScriptSet((HrnLibSsh2 [])
        {
            ...
            {.function = HRNLIBSSH2_SFTP_FSYNC, .resultInt = LIBSSH2_ERROR_EAGAIN},
            {.function = HRNLIBSSH2_SFTP_FSYNC, .resultInt = LIBSSH2_ERROR_NONE},
            {.function = HRNLIBSSH2_SFTP_FSTAT_EX, .param = "[1]", .resultInt = LIBSSH2_ERROR_EAGAIN, .sleep = 18},
            {.function = HRNLIBSSH2_SFTP_FSTAT_EX, .param = "[1]", .resultInt = LIBSSH2_ERROR_EAGAIN, .sleep = 5},
            HRNLIBSSH2_MACRO_SHUTDOWN()
        });
        ...
        TEST_ASSIGN(
            file,
            storageNewWriteP(
                storageTest, fileName, .user = TEST_USER_STR, .noAtomic = true, .group = TEST_GROUP_STR, .timeModified = 1),
            "new write file (defaults)");
        TEST_RESULT_VOID(ioWriteOpen(storageWriteIo(file)), "open file");
        TEST_RESULT_INT(ioWriteFd(storageWriteIo(file)), -1, "check write fd");
        TEST_RESULT_VOID(ioWriteClose(storageWriteIo(file)), "close file");
        ...
        TEST_ASSIGN(
            ...
            storageNewWriteP(
                storageTest, fileName, .user = TEST_USER_STR, .noAtomic = true, .group = TEST_GROUP_STR, .timeModified = 1,
                .noSyncPath = false, .noTruncate = true),
            "new write file");
        TEST_RESULT_VOID(ioWriteOpen(storageWriteIo(file)), "open file");
        TEST_RESULT_INT(ioWriteFd(storageWriteIo(file)), -1, "check write fd");
    ...
    if (testBegin("StorageWrite"))
    {
        ...
        storageTest = storageSftpNewP(
            TEST_PATH_STR, STRDEF("localhost"), 22, 20, 20, .user = TEST_USER_STR, .keyPriv = KEYPRIV, .keyPub = KEYPUB,
            .write = true, .hostkeyHash = hashTypeMd5);
        ...
        hrnLibSsh2ScriptSet((HrnLibSsh2 [])
        {
            ...
            {.function = HRNLIBSSH2_SFTP_STAT_EX, .param = "[\"" TEST_PATH "/no-truncate\",1]", .resultInt = LIBSSH2_ERROR_NONE,
                ...
                .flags = LIBSSH2_SFTP_ATTR_PERMISSIONS | LIBSSH2_SFTP_ATTR_ACMODTIME | LIBSSH2_SFTP_ATTR_UIDGID |
                    LIBSSH2_SFTP_ATTR_SIZE,
                .mtime = 77777, .uid = TEST_USER_ID, .gid = TEST_GROUP_ID},
            {.function = HRNLIBSSH2_SFTP_STAT_EX, .param = "[\"" TEST_PATH "/no-truncate\",1]", .resultInt = LIBSSH2_ERROR_NONE,
                .attrPerms = LIBSSH2_SFTP_S_IFDIR | LIBSSH2_SFTP_S_IRUSR | LIBSSH2_SFTP_S_IWUSR,
                .flags = LIBSSH2_SFTP_ATTR_PERMISSIONS | LIBSSH2_SFTP_ATTR_ACMODTIME | LIBSSH2_SFTP_ATTR_UIDGID |
                    LIBSSH2_SFTP_ATTR_SIZE,
