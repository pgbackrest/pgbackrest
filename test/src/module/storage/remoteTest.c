...
testRun(void)
{
    ...
    if (testBegin("storageNewRead()"))
    {
        ...
        ioFilterGroupAdd(filterGroup, cipherBlockNewP(cipherModeEncrypt, cipherInfoNewStore(cipherTypeAes256Cbc, STRDEF("x"))));
        ioFilterGroupAdd(filterGroup, cipherBlockNewP(cipherModeDecrypt, cipherInfoNewStore(cipherTypeAes256Cbc, STRDEF("x"))));
