    ...
    if (testBegin("StorageAzure, StorageReadAzure, and StorageWriteAzure with HTTPS"))
    {
        HRN_FORK_BEGIN()
        {
            ...
            HRN_FORK_PARENT_BEGIN(.prefix = "azure client")
            {
                ...
                testRequestP(service, HTTP_VERB_GET, "/file.txt", .range = "35-37");
                testResponseP(service, .content = "YY-");

                testRequestP(service, HTTP_VERB_GET, "/file.txt", .range = "50-85");
                testResponseP(service, .content = "ZZZ-!!M-S-!!01234567890123456789-!T-");

                testRequestP(service, HTTP_VERB_GET, "/file2.txt");
    ...
    if (testBegin("StorageAzure, StorageReadAzure, and StorageWriteAzure with HTTP"))
    {
        HRN_FORK_BEGIN()
        {
            ...
            HRN_FORK_PARENT_BEGIN()
            {
                ...
                // Shrink the block buffer to make sure it is resized to hold the entire input buffer
                TEST_RESULT_VOID(bufResize(((StorageWriteAzure *)write->driver)->blockBuffer, 1), "resize block buffer to 1");
