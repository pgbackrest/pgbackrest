    ...
    if (testBegin("StorageAzure, StorageReadAzure, and StorageWriteAzure"))
    {
        HRN_FORK_BEGIN()
        {
            ...
            HRN_FORK_PARENT_BEGIN(.prefix = "azure client")
            {
                ...
                testRequestP(service, HTTP_VERB_GET, "/file.txt", .range = "44-");
                testResponseP(service, .content = "this is a sample file");

                TEST_RESULT_STR_Z(
                    strNewBuf(storageGetP(storageNewReadP(storage, STRDEF("file.txt"), .offset = 44))), "this is a sample file",
                    "get file");

                // -----------------------------------------------------------------------------------------------------------------
                TEST_TITLE("get file with limit");

                testRequestP(service, HTTP_VERB_GET, "/file.txt", .range = "0-20");
                testResponseP(service, .content = "this is a sample file");

                TEST_RESULT_STR_Z(
                    strNewBuf(storageGetP(storageNewReadP(storage, STRDEF("file.txt"), .limit = VARUINT64(21)))),
                    "this is a sample file", "get file");
