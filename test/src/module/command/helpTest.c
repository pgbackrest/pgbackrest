...
testRun(void)
{
    ...
    if (testBegin("helpRender()"))
    {
        ...
        commandHelp = strZ(strNewFmt(
            ...
            "selected for --type=lsn if there is no match. For other types of PITR, e.g.\n"
            "xid, the --set option must be provided if the target is prior to the latest\n"
            "backup. See Point-in-Time Recovery for more details and examples.\n"
            "\n"
