...
testRun(void)
{
    ...
    if (testBegin("helpRender()"))
    {
        ...
        commandHelp = strZ(strNewFmt(
            ...
            "\n"
            "Replication slots are not included per recommendation of the PostgreSQL\n"
            "documentation which notes that if they were ...the subsequent use of the backup\n"
            "to create a standby may result in indefinite retention of WAL files on the\n"
            "standby, and possibly bloat on the primary if hot standby feedback is\n"
            "enabled... Even if the backup is only intended for use in creating a new\n"
            "primary, copying the replication slots isn't expected to be particularly\n"
            "useful, since the contents of those slots will likely be badly out of date by\n"
            "the time the new primary comes on line... See the PostgreSQL documentation:\n"
            "Making a Base Backup Using the Low Level API (subsection Backing Up The Data\n"
            "Directory) for more information regarding the exclusion of replication slots\n"
            "and certain other directories and files.\n"
            "\n"
            "Command Options:\n"
            "\n"
