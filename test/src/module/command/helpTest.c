...
testRun(void)
{
    ...
    if (testBegin("helpRender()"))
    {
        ...
        commandHelp = strZ(strNewFmt(
            ...
            "backup can be found, the latest backup from the first repository containing\n"
            "backups will be used. For other types of PITR, e.g. xid, the --set option must\n"
            "be provided if the target is prior to the latest backup. See Point-in-Time\n"
