...
testRun(void)
{
    ...
    if (testBegin("cmdStop()"))
    {
        ...
        TEST_RESULT_BOOL(storageExistsNP(storageTest, strNewFmt("%s/all" STOP_FILE_EXT, strPtr(lockPath))), true,
        ...
        // -------------------------------------------------------------------------------------------------------------------------
        TEST_RESULT_VOID(
            storagePutNP(storageNewWriteP(storageTest, strNewFmt("%s/bad" LOCK_FILE_EXT, strPtr(lockPath)), .modeFile = 0222), NULL),
        ...
        // -------------------------------------------------------------------------------------------------------------------------
        TEST_RESULT_VOID(storagePutNP(storageNewWriteNP(storageTest, strNewFmt("%s/empty" LOCK_FILE_EXT, strPtr(lockPath))), NULL),
            "create empty lock file");
        TEST_RESULT_VOID(cmdStop(), "    stanza, create stop file, force - empty lock file");
        TEST_RESULT_BOOL(storageExistsNP(storageTest, stanzaStopFile), true, "    stanza stop file created");
        TEST_RESULT_BOOL(storageExistsNP(storageTest, strNewFmt("%s/empty" LOCK_FILE_EXT, strPtr(lockPath))), false,
        ...
        TEST_RESULT_VOID(storageRemoveNP(storageTest, stanzaStopFile), "remove stop file");
        TEST_RESULT_VOID(
            storagePutNP(storageNewWriteNP(storageTest, strNewFmt("%s/badpid" LOCK_FILE_EXT, strPtr(lockPath))), BUFSTRDEF("32768")),
