...
testRun(void)
{
    ...
    if (testBegin("lock*()"))
    {
        ...
        TEST_ERROR(
            lockReadFileData(STRDEF(BOGUS_STR), 999999999), FileReadError,
            "unable to read lock file 'BOGUS': unable to read from BOGUS: [9] Bad file descriptor");
        TEST_RESULT_UINT(lockReadFileP(STRDEF(BOGUS_STR)).status, lockReadStatusMissing, "missing lock file");

        HRN_STORAGE_PUT_EMPTY(storageTest, "unlocked.lock");
        TEST_RESULT_UINT(
            lockReadFileP(STRDEF("unlocked.lock"), .remove = true).status, lockReadStatusUnlocked, "unlocked lock file");

        // -------------------------------------------------------------------------------------------------------------------------
        TEST_TITLE("read lock file (no data set)");

        LockReadResult lockReadResult;
        TEST_ASSIGN(lockReadResult, lockReadFileP(lockFile1Name), "read lock file");
        ...
        TEST_RESULT_UINT(lockReadFileP(lockFile1Name).status, lockReadStatusInvalid, "invalid lock file");

        // -------------------------------------------------------------------------------------------------------------------------
        TEST_TITLE("write errors");

        TEST_ERROR(lockWriteDataP(STRDEF(BOGUS_STR)), AssertError, "lock file 'BOGUS' not found");
