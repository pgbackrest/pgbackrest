...
testRun(void)
{
    ...
    if (testBegin("Lz4Compress and Lz4Decompress"))
    {
        ...
        TEST_ASSIGN(
            compressed, testCompress(lz4CompressNewVar(compressParamList), decompressed, 1024, 1024),
            "large in/large out buffer");

        TEST_RESULT_BOOL(
            bufEq(compressed, testCompress(lz4CompressNew(3), decompressed, 1024, 1)), true,
            "large in/small out buffer");

        TEST_RESULT_BOOL(
            bufEq(compressed, testCompress(lz4CompressNew(3), decompressed, 1, 1024)), true,
            "compress small in/large out buffer");

        TEST_RESULT_BOOL(
            bufEq(compressed, testCompress(lz4CompressNew(3), decompressed, 1, 1)), true,
        ...
        // TEST_RESULT_BOOL(
        //     bufEq(decompressed, testDecompress(lz4DecompressNewVar(decompressParamList), compressed, 1024, 1024)), true,
        ...
        //
        // TEST_ERROR(testDecompress(lz4DecompressNew(true), bufNew(0), 1, 1), FormatError, "unexpected eof in compressed data");
        ...
        // TEST_ERROR(testDecompress(lz4DecompressNew(false), truncated, 512, 512), FormatError, "unexpected eof in compressed data");
        ...
        //
        // TEST_ASSIGN(
        //     compressed, testCompress(lz4CompressNew(3), decompressed, bufSize(decompressed), 1024),
    ...
    if (testBegin("lz4DecompressToLog() and lz4CompressToLog()"))
    {
        ...
        //
        // TEST_RESULT_STR_Z(lz4DecompressToLog(decompress), "{inputSame: false, done: false, availIn: 0}", "format object");
        //
        // decompress->inputSame = true;
        // decompress->done = true;
        // TEST_RESULT_STR_Z(lz4DecompressToLog(decompress), "{inputSame: true, done: true, availIn: 0}", "format object");
