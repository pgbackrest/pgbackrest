...
build says nothing about which file the harness was working on."""
...
def file_read(path, ignore_missing=False):
    ...
    try:
        ...
        raise TestError("unable to open file '%s' for read" % path)
    except OSError as error:
        raise TestError("unable to open file '%s' for read: %s" % (path, error.strerror))


####################################################################################################################################
def file_write(path, content):
    """Write a file as text, creating parent paths as needed."""
    ...
    try:
        ...
        raise TestError("unable to open file '%s' for write: %s" % (path, error.strerror))
...
def file_remove(path, error_on_missing=False):
    ...
    try:
        ...
        if error_on_missing:
            raise TestError("unable to remove missing file '%s'" % path)
    except OSError as error:
        raise TestError("unable to remove file '%s': %s" % (path, error.strerror))
...
def path_create(path, mode=0o750):
    ...
    try:
        ...
        raise TestError("unable to create path '%s': %s" % (path, error.strerror))
...
def path_list(path, expression=None, error_on_missing=False):
    ...
    try:
        ...
        if error_on_missing:
            raise TestError("unable to list missing path '%s'" % path)

        return []
    except OSError as error:
        raise TestError("unable to list path '%s': %s" % (path, error.strerror))
