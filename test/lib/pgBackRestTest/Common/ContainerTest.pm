        ...
        if (!$bDeprecated)
        {
            ...
            else
            {
                ...
                if (vmPgRepo($strVm))
                {
                    $strScript .=
                        ...
                        ($strOS eq VM_U22 && hostArch() eq 'x86_64' ?
                ...
                $strScript .=
                    ...
                    ($strOS eq VM_U22 && hostArch() eq 'x86_64'
                        ? " -t \$(lsb_release -s -c)-pgdg-snapshot" : '') . " postgresql-common libpq-dev && \\\n" .
            ...
            if (defined($oOS->{&VM_DB}) && @{$oOS->{&VM_DB}} > 0)
            {
                ...
                else
                {
                    $strScript .=
                       "    apt-get install -y --no-install-recommends" .
                        ($strOS eq VM_U22 && hostArch() eq 'x86_64' ? " -t \$(lsb_release -s -c)-pgdg-snapshot" : '');
                  }
