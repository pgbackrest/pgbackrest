...
sub run
{
    ...
    if ($self->begin("storageRepo()"))
    {
        ...
        #---------------------------------------------------------------------------------------------------------------------------
        $self->testResult(
            sub {storageRepo()->pathGet(STORAGE_REPO_BACKUP_TMP)},
            $self->testPath() . '/repo/temp/db.tmp', 'check backup temp path');
        $self->testResult(
            sub {storageRepo()->pathGet(STORAGE_REPO_BACKUP_TMP . '/file')}, $self->testPath() . '/repo/temp/db.tmp/file',
            'check backup temp path');
