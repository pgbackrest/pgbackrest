...
sub run
{
    ...
    if ($self->begin('read()'))
    {
        ...
        $oFileIo = $self->testResult(
            sub {new pgBackRest::Storage::Posix::File(
                $oDriver, $self->dataPath() . '/filecopy.archive2.bin', STORAGE_FILE_READ)},
            '[object]', 'open read');
    ...
    if ($self->begin('write()'))
    {
        ...
        #---------------------------------------------------------------------------------------------------------------------------
        my $oFileIo = $self->testResult(
            sub {new pgBackRest::Storage::Posix::File($oDriver, $strFile, STORAGE_FILE_WRITE, {bAtomic => true})},
            '[object]', 'open write');
