...
sub run
{
    ...
    (
        ...
        {vm => VM3, remote =>  true, tls => false, storage => AZURE, encrypt =>  true, compress => LZ4},
        {vm => VM4, remote => false, tls => false, storage =>    S3, encrypt => false, compress =>  GZ},
        {vm => VM4, remote =>  true, tls =>  true, storage =>   GCS, encrypt =>  true, compress => ZST},
    )
    {
        ...
        my ($oHostDbPrimary, $oHostDbStandby, $oHostBackup) = $self->setup(
            true, $self->expect(), {bHostBackup => $bRemote, bTls => $bTls, strStorage => $strStorage, bRepoEncrypt => $bEncrypt,
