...
eval
{
    ...
    if (!defined($iVmId))
    {
        ...
        if (($strVm eq VM_ALL || $strVm eq $strVmHost)  && !$bDryRun)
        {
            ...
            executeTest(LIB_COVER_EXE . " -report json -outputdir ${strBackRestBase}/test/coverage ${strCoveragePath}_temp");
            executeTest("rm -rf ${strCoveragePath}_temp");
            executeTest("cp -rp ${strCoveragePath} ${strCoveragePath}_temp");
            executeTest(LIB_COVER_EXE . " -outputdir ${strBackRestBase}/test/coverage ${strCoveragePath}_temp");
            executeTest("rm -rf ${strCoveragePath}_temp");
